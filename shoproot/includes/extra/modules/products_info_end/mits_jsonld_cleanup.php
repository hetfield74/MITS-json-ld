...
if (defined('MODULE_MITS_JSON_LD_STATUS')
  ...
  && isset($product->data['products_description'])
  && !empty($product->data['products_description'])
  && mits_jsonld_custom_enabled()
) {
    list($nodes, $cleanHtml) = mits_jsonld_extract_and_strip_from_text($product->data['products_description']);
    ...
    $product->data['products_description'] = $cleanHtml;
    $info_smarty->assign('PRODUCTS_DESCRIPTION', $cleanHtml);
}